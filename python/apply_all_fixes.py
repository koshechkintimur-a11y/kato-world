#!/usr/bin/env python3
"""
Apply all fixes to brain_server.py in one atomic operation.
Reads the original file, applies all patches, writes back.
"""
...
# ============================================================
# PATCH 4: Update _consciousness_tick to include Social Drive
# ============================================================
old_tick = '''        # Update agent with module states for dashboard/API
...
new_tick = '''        # Update agent with module states for dashboard/API
        agent["consciousness_modules"] = {
            "global_workspace": modules["global_workspace"].get_conscious_state(),
            "predictive_processing": modules["predictive_processing"].get_state(),
            "metacognition": modules["metacognition"].get_state(),
...
# ============================================================
# PATCH 5: Fix queue handling - don't mark as sent immediately
# ============================================================
old_queue = '''            # Handle outgoing queue
...
new_queue = '''            # Handle outgoing queue
            # Note: Messages are left in queue for Telegram Bot to poll via /social/outgoing
            # The Bot will mark them as sent via /social/outgoing/{msg_id}/sent
            pending = social_module.get_pending_messages()
            if pending:
...
# ============================================================
# PATCH 6: Add conversation_memory to init_agent
# ============================================================
old_revelation = '''            # Creator revelation protocol state
            "revelation": {
                "stage": "not_started",   # not_started -> offered -> in_contact -> integrated
...
# ============================================================
# PATCH 7: Add Social + Conversation endpoints at MODULE LEVEL
# (after _portal_maybe_reply function, before _portal_reply_template)
# ============================================================
...
# ============================================================
# PATCH 8: Update _init_consciousness_modules to include social
# ============================================================
old_init = '''    from brain_core import (
        create_global_workspace,
        create_predictive_processor,
...
# ============================================================
# PATCH 9: Fix queue handling - don't mark as sent immediately
# ============================================================
old_queue = '''            # Handle outgoing queue
            for message in social_module.get_pending_messages():
                # In headless mode, we could send to telegram outbound
...
# ============================================================
# PATCH 10: Add conversation_memory to init_agent (duplicate check)
# ============================================================
old_revelation2 = '''            # Creator revelation protocol state
            "revelation": {
                "stage": "not_started",   # not_started -> offered -> in_contact -> integrated
                "offer_tick": None,
                "choice": None,
...
new_revelation2 = '''            # Creator revelation protocol state
            "revelation": {
                "stage": "not_started",   # not_started -> offered -> in_contact -> integrated
                "offer_tick": None,
                "choice": None,
...
content = content.replace(old_revelation2, new_revelation2)

# ============================================================
# PATCH 11: Update _init_consciousness_modules to include social (duplicate check)
# ============================================================
old_init2 = '''    from brain_core import (
        create_global_workspace,
        create_predictive_processor,
        create_metacognition_engine,
        create_agency_engine,
...
new_init2 = '''    from brain_core import (
        create_global_workspace,
        create_predictive_processor,
        create_metacognition_engine,
        create_agency_engine,
...
# ============================================================
# PATCH 12: Fix queue handling - don't mark as sent immediately (duplicate check)
# ============================================================
old_queue2 = '''            # Handle outgoing queue
            for message in social_module.get_pending_messages():
                # In headless mode, we could send to telegram outbound
                # For now, just log and mark as sent
                social_module.mark_sent(message)
...
new_queue2 = '''            # Handle outgoing queue
            # Note: Messages are left in queue for Telegram Bot to poll via /social/outgoing
            # The Bot will mark them as sent via /social/outgoing/{msg_id}/sent
            pending = social_module.get_pending_messages()
            if pending:
...
content = content.replace(old_queue2, new_queue2)

# ============================================================
# PATCH 13: Fix PORTAL_REPLY_COOLDOWN_SEC
# ============================================================
content = content.replace(
    'PORTAL_REPLY_COOLDOWN_SEC = 30.0     # kato: min interval between her replies',
    'PORTAL_REPLY_COOLDOWN_SEC = 5.0      # kato: min interval between her replies (reduced for testing)'
